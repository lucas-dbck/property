...
function backendToInputValues(data: Record<string, unknown> = {}): InputValues {
  ...
  return {
    ...
    operating_cost_rate: rateToPercentInput(firstValue(data, ["operating_cost_rate", "operatingCostRate"]), 10),
    vacancy_rate: rateToPercentInput(firstValue(data, ["vacancy_rate", "vacancyRate"]), 0),
    inflation_rate: rateToPercentInput(firstValue(data, ["inflation_rate", "inflationRate"]), 1.5),
    down_payment: downPayment !== undefined ? toNumber(downPayment) : 0,
    interest_rate: toNumber(firstValue(data, ["interest_rate", "interestRate"]), 2.5),
...
function inputToBackendData(values: InputValues): Record<string, unknown> {
  ...
  return {
    ...
    operating_cost_rate: percentInputToDecimal(firstValue(values, ["operating_cost_rate", "operatingCostRate"]), 10),
    vacancy_rate: percentInputToDecimal(firstValue(values, ["vacancy_rate", "vacancyRate"]), 0),
    inflation_rate: percentInputToDecimal(firstValue(values, ["inflation_rate", "inflationRate"]), 1.5),
  ...
  const message = status === "failed"
    ...
    : aiStatus === "not_configured"
      ...
      : missing.length > 0
        ...
        : "Review the imported values before trusting leveraged ROI."
...
function backendAnalysisToFrontend(response: BackendAnalysis): AnalyzeResponse {
  ...
  return {
    ...
    metrics: [
      ...
      metric("netYield", "Net yield", a.net_yield, "percent", "Property-level return before financing: net operating income / total project cost."),
